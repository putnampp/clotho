...
BOOST_AUTO_TEST_CASE( row_grouped_crossover_test ) {
    ...
    config.put( "recombination.sequence_bias.p", 1.0);
...
//    boost::property_tree::write_json( std::cout, config );
    ...
    // and the event will occur at 0.562272
    ...
    parents.getAlleleSpace().setPositionAt( 22, 0.5622 );   // allele same bin as event, but before the event
    parents.getAlleleSpace().setPositionAt( 15, 0.562273 );  // allele same bin as event, but after the event
