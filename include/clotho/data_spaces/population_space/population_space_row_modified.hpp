...
#ifndef CLOTHO_POPULATION_SPACE_ROW_MODIFIED_HPP_
#define CLOTHO_POPULATION_SPACE_ROW_MODIFIED_HPP_
...
class population_space_row_modified {
public:
    ...
    weight_type getWeight( unsigned int offset ) const {
        if( offset < m_genome_traits.size() )
            return m_genome_traits[ offset ];
        else
            return 1.0;
    }
    ...
    const_weight_iterator begin_genome_traits( unsigned int idx ) const {
        ...
        return ((idx < m_genome_traits.size()) ? m_genome_traits.begin() + idx : m_genome_traits.end());
    }

    const_weight_iterator end_genome_traits( unsigned int idx ) const {
        idx = (idx + 1) * m_trait_count;

        return ((idx < m_genome_traits.size()) ? m_genome_traits.begin() + idx : m_genome_traits.end());
    }

    bool isSequenceModified( unsigned int offset ) const {
#ifdef DEBUGGING
        assert( offset < m_modified.size() );
#endif  // DEBUGGING

        return (m_modified[ offset ] >= 0);
    }

    int getSequenceModified( unsigned int offset ) const {
#ifdef DEBUGGING
        assert( offset < m_modified.size() );
#endif  // DEBUGGING
        return m_modified[ offset ];
    }

    void updateSequenceModified( unsigned int offset, int modified_idx  ) {
#ifdef DEBUGGING
        assert( offset < m_modified.size() );
#endif  // DEBUGGING
        m_modified[ offset ] = modified_idx;
    }

    template < class Iterator >
    void updateMutatedSequences( Iterator first, Iterator last ) {
    ...
    virtual ~population_space_row_modified() {
...
protected:
    ...
    void resize( ) {
        ...
        while( m_modified.size() < m_genome_rows ) {
...
#endif  // CLOTHO_POPULATION_SPACE_ROW_MODIFIED_HPP_
