...
#ifndef CLOTHO_POPULATION_SPACE_COLUMNAR_HPP_
#define CLOTHO_POPULATION_SPACE_COLUMNAR_HPP_
...
class population_space_columnar {
public:
    ...
    population_space_columnar() :
        m_genetic_space( NULL )
        , m_genetic_space_allocated(0)
        , m_genome_columns(0)
        , m_allele_block_rows(0)
        , m_allele_count(0)
        , m_trait_count(0)
    {}

    genome_pointer getHaploidGenome( unsigned int offset ) {
        assert( offset < m_genome_columns );
        return m_genetic_space + offset;
    }
    ...
    void setMaxAlleles( unsigned int A ) {
        ...
        m_allele_block_rows = (m_allele_count / bit_helper_type::BITS_PER_BLOCK) + ((m_allele_count % bit_helper_type::BITS_PER_BLOCK > 0) ? 1 : 0);
    }
    ...
    void mutate( unsigned int seq_idx, unsigned int all_idx ) {
        unsigned int block_row = (all_idx / bit_helper_type::BITS_PER_BLOCK);
        unsigned int bit_offset= (all_idx % bit_helper_type::BITS_PER_BLOCK);

#ifdef DEBUGGING
        assert( seq_idx < m_genome_columns );
        assert( all_idx < m_allele_count);

        assert( (m_genetic_space[ block_row * m_genome_columns + seq_idx ] & ((block_type) 1 << bit_offset)) == (block_type)0);
#endif  // DEBUGGING

        m_genetic_space[ block_row * m_genome_columns + seq_idx ] ^= ((block_type) 1 << bit_offset);
    }
    ...
    void remove_fixed_allele( unsigned int all_idx ) {
        ...
        block_type * first = m_genetic_space + (block_row * m_genome_columns);
        block_type * last = first + m_genome_columns;
    ...
    row_pointer begin_block_row( unsigned int idx ) {
        return m_genetic_space + idx * m_genome_columns;
    }

    row_pointer end_block_row( unsigned int idx ) {

        return m_genetic_space + (idx + 1) * m_genome_columns;
    }

    genome_pointer begin_genome( unsigned int idx ) {
        assert( idx < m_genome_columns );
    ...
    genome_pointer end_genome( unsigned int idx ) {
        ...
        return m_genetic_space + m_allele_block_rows * m_genome_columns + idx;
    }

    void updateGenomeWeights( unsigned int genome_index, weight_vector & weights ) {
#ifdef DEBUGGING
        assert( genome_index < m_genome_columns );
    ...
    virtual ~population_space_columnar() {
...
protected:
    ...
    void resize( ) {
        ...
        size_t  new_total = m_genome_columns * m_allele_block_rows;
        if( new_total > m_genetic_space_allocated ) {
            if( m_genetic_space != NULL ) {
                delete [] m_genetic_space;
            }

            new_total += 10 * m_genome_columns;  // pad each allocation by an additional 10 rows
            m_genetic_space = new block_type[ new_total ];

            m_genetic_space_allocated = new_total;
        }

        new_total = m_trait_count * m_genome_columns;
    ...
    unsigned int    m_genome_columns, m_allele_block_rows, m_allele_count, m_trait_count;
...
#endif  // CLOTHO_POPULATION_SPACE_COLUMNAR_HPP_
