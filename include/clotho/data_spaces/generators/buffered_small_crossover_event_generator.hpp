...
#ifndef CLOTHO_BUFFERED_SMALL_CROSSOVER_EVENT_GENERATOR_HPP_
#define CLOTHO_BUFFERED_SMALL_CROSSOVER_EVENT_GENERATOR_HPP_
...
class buffered_small_crossover_event_generator {
public:
    ...
    buffered_small_crossover_event_generator( random_engine_type * rng, boost::property_tree::ptree & config ) :
        m_rand( rng )
        , m_pos( NULL )
        , m_pos_size(0)
        , m_events_size(0)
...
/**
 *
 * Copies a subsequence of the parent position vector into local memory
 *
 */
    void update_buffer( unsigned int offset ) {
        memcpy( m_pos_buffer, m_pos + offset, BUFFER_MAX * sizeof( position_type ) );
    }

/**
 *
 * Generates an ordered list of crossover events 
 *
...
/**
 *
 * test whether the genetic position at the given buffer relative index
 * is in a crossover region
 *
 */
    inline bool operator()( const size_t index ) const {
        assert( index < BUFFER_MAX );
#ifdef DEBUGGING
        if( index >= m_pos_size ) {
        ...
        position_type p = m_pos_buffer[ index ];
        return eval( p );
    }
    ...
    virtual ~buffered_small_crossover_event_generator() { }
...
protected:
    ...
    position_type       m_pos_buffer[ BUFFER_MAX ];
...
#endif  // CLOTHO_BUFFERED_SMALL_CROSSOVER_EVENT_GENERATOR_HPP_
