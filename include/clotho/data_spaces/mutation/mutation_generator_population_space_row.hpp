...
#ifndef CLOTHO_MUTATION_GENERATOR2_POPULATION_SPACE_ROW_HPP_
#define CLOTHO_MUTATION_GENERATOR2_POPULATION_SPACE_ROW_HPP_
...
class MutationGenerator2 < RNG, population_space_row< BlockType, WeightType > > {
public:
    typedef RNG     random_number_engine;

    typedef population_space_row< BlockType, WeightType >   sequence_space_type;
    ...
    inline void operator()( sequence_space_type * seqs, unsigned int seq_idx, unsigned int all_idx ) {
        //std::cerr << "Mutating " << seq_idx << " - " <<  all_idx << std::endl;
...
#endif  // CLOTHO_MUTATION_GENERATOR2_POPULATION_SPACE_ROW_HPP_
