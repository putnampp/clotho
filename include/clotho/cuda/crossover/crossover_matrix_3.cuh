...
//    __shared__ RealType local_alleles[ THREAD_NUM ];
    ...
    while( seq_idx < nSequences ) {
        ...
        //rand_pool[tid] = curand_mtgp32_single_specific( localState, tid, THREAD_NUM ) - 1.0;    // -1.0 b/c of bug in curand library code
//        unsigned int rand = curand_mtgp32_specific( &states[blockIdx.x], tid, THREAD_NUM );

        unsigned int rand = 0;
        if( tid < THREAD_NUM ) {
            // MTGP32 only allows for 256 threads/state
            rand = curand( &states[blockIdx.x] );
        }
        __syncthreads();
...
#ifdef USE_TEXTURE_MEMORY_FOR_ALLELE
            RealType local_allele = tex1Dfetch( allele_tex, all_idx );
#else
            RealType local_allele = allele_list[all_idx];
#endif  // USE_TEXTURE_MEMORY_FOR_ALLELE

            if( tid < blockDim.x / 32 ) {   // 1024 / 32 == 32
                seq[ tid ] = rand;
            }
            __syncthreads();

            all_idx += blockDim.x;
            seq += blockDim.x / 32; // (1024 / 32) == 8
        }
...
void crossover< 3 >::initialize( ) {
    ...
    std::cerr << "Initialized random states" << std::endl;
}
...
#ifdef USE_TEXTURE_MEMORY_FOR_ALLELE
    cudaBindTexture(0, allele_tex, allele_list, nAlleles * sizeof(allele_type));
#endif  // USE_TEXTURE_MEMORY_FOR_ALLELE

    crossover_kernel_3<<< BLOCK_NUM_MAX, 1024 >>>( dStates, allele_list, event_list, sequences, nSequences, nAlleles, sequence_width );
}
