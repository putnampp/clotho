...
template < class RealType, class IntType >
__device__ void _resize_allele_space( device_allele_space< RealType, IntType, unordered_tag > * aspace, unsigned int N ) {
    ...
    if( aspace->capacity < N ) {
        ...
        unsigned int free_size = compute_free_list_size<typenam space_type::int_type >( N );
...
__global__ void _merge_allele_space( device_allele_space< RealType, IntType, unordered_tag > * aspace
    ...
    if( (tid == 0) && (output.size < min_out_size) ) {
        // resize output
        _resize_allele_space( &output, min_out_size );
    }
    __syncthreads();
