...
#ifndef ADJACENCY_VECTOR_HPP_
#define ADJACENCY_VECTOR_HPP_

#include "clotho/powerset/adjacency_matrix.hpp"
...
public:
    ...
    adjacency_vector( pointer start, pointer end, size_type soft_max_index ) :
        m_start( start )
        , m_end( end )
        , m_soft_max_index( soft_max_index )
    {}
    ...
    pointer soft_end() {
        return m_start + (m_soft_max_index / bit_helper_type::BITS_PER_BLOCK + 1);
    }

    virtual ~adjacency_vector() {}

protected:
    pointer     m_start, m_end;
    size_type   m_soft_max_index;
};

template < class Element, class Block, class SizeType, class BlockMap, class ElementKeyer >
class powerset< Element, adjacency_vector< Block, SizeType >, Block, BlockMap, ElementKeyer > {
public:
    ...
    typedef adjacency_vector< block_type, SizeType >                subset_type;
    typedef std::shared_prt< subset_type >                          subset_ptr;
    typedef adjacency_matrix< block_type, SizeType >                bitset_type;

    typedef adjacency_matrix< block_type, SizeType >                family_type;
    typedef family_type::row_pointer                                family_iterator;
    typedef family_type::row_pointer                                cfamily_iterator;
...
#define TEMPLATE_HEADER template < class Element, class Block, class SizeType, class BlockMap, class ElementKeyer >
#define POWERSET_SPECIALIZATION powerset< Element, adjacency_vector< Block, SizeType>, Block, BlockMap, ElementKeyer >

TEMPLATE_HEADER
POWERSET_SPECIALIZATION::powerset() : m_family( 0, 0 ) {}
