...
def make_device_heap( s=1073741824 ):
    return { "heap" : { "malloc": { "size": "{0}".format(s) } } }

def make_state_pool():
    return { "max_blocks" : "128", "max_threads": "1" }

def make_allele_frequency_kernel():
    return { "kernel": { "blocks": { "x": "200", "y": "1" }, "threads": { "x":"32", "y": "32"}}}

def make_pairwise_difference_kernel():
    return { "kernel": { "version": "0", "threads": { "x": "32", "y": "32", "z": "1"}}}
...
def make_cuda_configuration(prefix):
    # generations
    G = [ "100000" ] 
    # population size
    P = [ "10000" ]
    ...
    p_neutrality = [ "0.0", "1.0" ]
    rho = [ "1.0", "10.0", "23.0", "35.0", "50.0", "70.0", "100.0" ]
    mu = [ "0.001", "0.01", "0.1", "1.0" ]

    # quadratic scale
    scale = [ "4.0" ]
    for g, p, n, r, s, m in itertools.product(G, P, p_neutrality, rho, scale, mu ):
        print("{3};{4};{0} x {1} x {2} ".format( n, m, r, g, p))
        config = make_base_configuration( g, p, n, m, r, None, 100 )
        heap_size = 2 * 1024 * 1024 * 1024
        config[ "device" ] = make_device_heap(heap_size)
        config[ "state_pool" ] = make_state_pool()
        config[ "crossover" ] = make_crossover()
        config[ "allele_frequency" ] = make_allele_frequency_kernel()
        config[ "pairwise_difference" ] = make_pairwise_difference_kernel()

        path="{0}neutral={1}/mu={2}/rho={3}".format( prefix, n, m, r )
        write_config( config, path)


make_cuda_configuration("")
